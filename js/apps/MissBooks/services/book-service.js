...
function addReview(bookId, review) {
    ...
    if (!review.name) review.name = 'Book Reader'
...
function searchBooks(str) {
    const searchInput = encodeURI(str)
    const url = `https://www.googleapis.com/books/v1/volumes?printType=books&q=${searchInput}`
    return fetch(url)
        .then(res => res.json())
        .then(data => {
